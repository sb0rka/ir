...
func Build(title, description string, c Context) string {
	...
	b.WriteString("1. Prefer `import_entity_events` when the task is to find events for one entity and add them to the graph. Call it once with the resolved `entity_id` (or type+value) and the suggested time_range. Use manual `add_investigation_agent_results` only for edges the tool cannot derive.\n")
	...
	b.WriteString("5. Truncated/partial ≠ absent. Follow `next_cursor`; if truncated without a cursor, narrow `time_range`/filters and retry. Inspect `source_states` and `source_errors`. To verify one known `source_event_id`, use `gateway_resolve_context`, not a search filter.\n")
	b.WriteString("6. Write only via `import_entity_events` or `add_investigation_agent_results`. For manual writes: declare Gateway imports in `events`/`entities` with batch-local `ref`, point `nodes` at them with `event_ref`/`entity_ref`, or use attached `event_id`/`entity_id`/`node_id`. Edges use batch-local node refs, `why`, and `evidence_event_refs`.\n")
	...
	b.WriteString("8. Final answer must quote write counts verbatim: for `import_entity_events` report `events_found` and `events_imported` (and `source_errors`/`warnings` when present); for other writes quote `events`/`nodes`/`edges`. Success requires explaining any gap where imported < found. Do not claim success after importing a hand-picked subset to satisfy events>0. If no successful write happened, say nothing was written.\n\n")
