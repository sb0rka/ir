...
Реализованных ручек пока нет — все возвращают 501. Когда появятся: `server`
не ходит в базу напрямую, только через интерфейс `store.Database`, а SQL
пишется инлайном в методе стора.

Единственный живой пример этого пути — `RoleBindings`: его зовёт middleware,
чтобы резолвить тенант и роли по субъекту токена.
