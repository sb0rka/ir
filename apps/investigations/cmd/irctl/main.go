// irctl — разовые операции над базой расследований, которые незачем держать
// в конфиге сервиса.
//
// Появился вместо INV_BOOTSTRAP_ADMIN_SUBJECTS: раздача первой роли нужна один
// раз на стенд, а переменная окружения живёт всё время работы процесса и даёт
// админа любому, чей subject в неё попал, — навсегда и молча.
package main
