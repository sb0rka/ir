...
- Ordinary search/context responses contain metadata and evidence references. Full payload and selected-file content is available only through the explicit [evidence export API](evidence.md). File export follows the captured `sources/getfile -> tasks/{id} -> download` ZIP chain. PCAP references are preserved, but exporting them returns `unsupported_capability` until the actual vendor chain is captured on the lab. No PCAP route is guessed.
