...
Project allowlists are process configuration: `PROJECT_SOURCE_ALLOWLISTS={"abcdef1234":["maxpatrol-siem","pt-sandbox"]}`. Only registered sources are accepted; projects absent from the map are denied. When authentication is enabled, `DATABASE_URI` must point to the Investigations database so Gateway can verify `role_bindings`.

For a smaller local dataset:

```bash
AUTH_DISABLED=true PROJECT_SOURCE_ALLOWLISTS='{"abcdef1234":["maxpatrol-siem","pt-sandbox"]}' \
