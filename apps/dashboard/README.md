...
2. **Граф + таймлайн** — `GET /investigations/{id}/graph` and `/events`. Node positions are computed locally and stored in `localStorage`. Dense graphs wrap chronological events left-to-right across rows; automatic fit stops at the canvas minimum zoom, so larger graphs may require panning. Use **Arrange nodes** to reset old saved positions. Timeline labels occupy non-overlapping lanes; scroll vertically within the timeline to reach crowded events. Every event remains individually selectable.
