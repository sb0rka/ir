...
`api/` — источник правды публичного Investigations API.
...
```bash
task spec       # api/bundle.yaml + Redocly lint
task spec-docs  # api/docs.html
task gen        # Go, TypeScript и заглушки обработчиков
```

`api/bundle.yaml`, `api/build/` и `api/docs.html` — производные файлы и в Git
не хранятся.
...
- Auth: Bearer JWT платформы с audience `api.local`; tenant выбирается обязательным
