...
paths/            девять доменов и их локальные схемы
shared/           переиспользуемые enum, параметры, ошибки и ответы
```

Домены: `investigations`, `hypotheses`, `events`, `entities`, `findings`,
`sessions`, `graph`, `reference`, `som`. В текущем контракте 40 путей и 58
операций. Реализован вертикальный срез страницы расследования, гипотезы как
именованные подграфы общего графа, атомарное сохранение выбранных данных из
Gateway и полный edge CRUD/review; остальные незаявленные операции отвечают
`501 not_implemented`.
