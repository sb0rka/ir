...
- Gateway остаётся владельцем поиска больших потоков для human REST; Investigations сначала
  создаёт пустое расследование, затем принимает выбранные `context` и
  `agent-results` отдельными запросами. Human-клиент может передать коды
  источников и исходные идентификаторы, а MCP-агент — только локальные UUID
  уже прикреплённых events/entities или существующих graph nodes.
